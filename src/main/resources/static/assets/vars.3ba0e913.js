import { g as getDevices, c as getPlcRegisterList, d as getPlcDataTypeMap, f as getTagDataTypeList, h as getTagGroupList } from './config.ca47f3a2.js';
import { k as service, _ as _export_sfc } from './index.acc5b702.js';
  ...
  setup() {
    ...
    const write = varInfo => {
      ...
      ElMessageBox.prompt('同步写入数据', '', {
        ...
        .then(({ value }) => {
          const { data, msg, code } = writeVar(varInfo, value);
          if (code == 200) {
            ElNotification({
              title: '写入成功',
              message: '写入成功',
